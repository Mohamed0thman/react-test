...
import { cookieClient } from '../../services/cookieClient';
...
const verifyAuth = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const accessToken = cookieClient.getCookie('access_token') || null;
      const user = await authAPI.verifyAuth(accessToken);
...
const login = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      if (userType !== userTypesEnum.CONSUMER)
        return rejectWithValue([
          { msg: 'Wrong email or password. Are you a property owner ?' },
        ]);
      cookieClient.setCookie('access_token', res.accessToken, 3);
      cookieClient.setCookie('refresh_token', res.refreshToken, 3);
      return res;
    } catch (e) {
      return rejectWithValue(e.response.data.errors);
    }
